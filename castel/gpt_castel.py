...
product_information = pd.read_csv('castel/pisos_castel.csv')
responses = []
...
for index, row in product_information.iterrows():
    ...
    if row['Descripcion'] == '[]':
        ...
        prompt = f"""
    Con el nombre del producto "{row['Producto']}", de la marca "Castel", y la descripcion "{row['Descripcion']}"

    Generar una descripcion nueva de 6 oraciones, en formato de parrafo, usando las siguientes palabras:
    "loseta para pisos ", "pisos cerámicos", "castel cerámica", "pisos de loseta", "loseta tipo madera", "baldosas de porcelanato",
    "azulejo económico", "mosaico", "exterior", "recámara", "marca castel","instalacion simplo"
...
df.to_csv("castel/pisos.csv", index=False)