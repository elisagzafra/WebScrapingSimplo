...
product_information = pd.read_csv('tendenzza/productos_tendenzza.csv')
responses = []
...
for index, row in product_information.iterrows():
    match row['Categoria']:
        case 'Piso':
            prompt = f"""
        Te voy a dar un producto y tu vas a generarle una descripción.
            
        El nombre del producto es "{row['Producto']}", de la marca "Tendenzza", y su descripción es "{row['Descripcion']}"

        Generar una descripcion nueva de 6 oraciones, en formato de parrafo, usando las siguientes palabras:
        "loseta para pisos ", "pisos cerámicos", "pisos de loseta", "loseta tipo madera", "baldosas de porcelanato",
        "azulejo económico", "mosaico", "exterior", "recámara", "instalacion de Simplo"
...
df.to_csv("tendenzza/responses.csv", index=False)