...
product_information = pd.read_csv('kwikset/productos_kana.csv')
responses = []
...
for index, row in product_information.iterrows():
    ...
    El nombre del producto es "{row['Producto']}", su descripción es "{row['Descripcion']}", y es de la marca "Kaná".

    Generar una descripcion de 6 oraciones, en formato de párrafo, usando las siguientes palabras:
    "muebles para baño", "muebles para lavabo de baño", "muebles de madera para baño", "muebles para lavamanos", 
    "muebles para baño con labavo", "comodas para baño", "instalación de Simplo"
...
df.to_csv("kwikset/muebles.csv", index=False)